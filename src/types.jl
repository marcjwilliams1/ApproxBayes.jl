...
type ABCSMCModel <: ABCtype
  ...
  new([ABCSMC(sim_func[i], nparams[i], ϵT, prior[i],  maxiterations = maxiterations, constants = constants[i], nparticles = nparticles, α = α, ϵ1 = ϵ1) for i in 1:length(sim_func)], length(sim_func), modelkern)
